...
# ///
"""ADW Quality — run a quality block (test/lint/typecheck/build).

Usage:
    uv run adws/adw_quality.py test|lint|typecheck|build [--config ...] [--adw-id a1b2c3d4]
...
VALID = ("test", "lint", "typecheck", "build")


def main(prompt: str, config: str = "adws/adw_sssf_config/sssf.config.yaml", adw_id: str | None = None) -> int:
    only = prompt.strip() if prompt.strip() in VALID else None
    ...
    with run.phase(PhaseParams(name="quality", kind="code", owner="quality",
                               description=f"Run the deterministic quality block(s)")) as ph:
        result = quality.run_quality(run, only=only)
