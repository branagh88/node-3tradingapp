...
class Tracer:
    ...
    def session_finish(self, adw_id: str, ok: bool) -> None:
        ...
        # Close any phase still marked 'running' — the run is over, so an open
        # phase is a phantom (process died mid-phase, killed run, crashed pi).
        # Without this the UI keeps a block "working" forever (the flashing
        # phases on d21ae368). The phase context manager normally does this on
        # a clean exception; this catches every other exit path.
        try:
            self.conn.execute(
                "UPDATE phases SET status='fail', ended_at=COALESCE(ended_at, ?), "
                "error=COALESCE(error, 'run ended while phase was open') "
                "WHERE adw_id=? AND status='running'",
                (now_iso(), adw_id),
            )
        except Exception:
            pass   # best-effort — phases may not exist on an old schema
