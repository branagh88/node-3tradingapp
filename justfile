...
# quick fix + test loop (no architect): just build-test "fix the jump physics"
build-test *ARGS:
...
# engine-aware specialist pipeline (auto-detect phaser/threejs/web/generic):
# architect -> builder -> test -> stack-guard. just sdlc "add a hub scene"
sdlc *ARGS:
    uv run adws/adw_sdlc.py --config {{config}} "$@"

# phaser/threejs aliases force their engine (thin wrappers over adw_sdlc)
phaser *ARGS:
    uv run adws/adw_phaser_sdlc.py --config {{config}} "$@"
...
# Three.js specialist pipeline: architect -> builder -> test -> stack-guard
# just threejs "add rapier physics to the world"
threejs *ARGS:
    uv run adws/adw_threejs_sdlc.py --config {{config}} "$@"

# Web site build (React/Vite/TS): just web "add a contact form to the home page"
web *ARGS:
    uv run adws/adw_prompt.py --config {{config}} --agent web-builder "$@"
